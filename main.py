# main.py → FLUJO COMPLETO CITAS MÉDICAS 2025 (YCloud + Supabase + Railway)
...
# ====================== ESTADO EN SUPABASE ======================
# Tabla: conversaciones (telefono TEXT PRIMARY KEY, estado TEXT, paso TEXT, datos JSONB)

async def get_estado(telefono: str):
    # Implementación mínima con dict (puedes mover a Supabase después)
    return app.state.conversaciones.get(telefono, {"estado": "inicio"})

async def set_estado(telefono: str, datos: dict):
    app.state.conversaciones[telefono] = datos

@app.on_event("startup")
async def startup():
    app.state.conversaciones = {}
...
async def webhook(request: Request):
    ...
    for msg in data["messages"]:
        ...
        if estado["estado"] == "inicio":
            ...
            if "1" in texto or "agendar" in texto:
            ...
            try:
                ...
                await enviar_mensaje(telefono, "Formato inválido. Usa DD-MM-AAAA")
...
@app.get("/")
async def root():
    return {"status": "Bot citas médicas 24/7 activo", "hora_chile": datetime.now(CHILE_TZ).strftime("%d-%m-%Y %H:%M")}